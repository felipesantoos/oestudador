...
export class AuthService {
  constructor() {
    // Initialize email service when first used
   ...
   */
  async resendVerificationEmailService(email) {
    // Find user by email
   ...
   * Login user
   */
  async loginUserService(email, password, rememberMe = false) {
    // Find user by email
    const user = await userRepository.findByEmail(email);
   ...
   */
  async invalidateTokenService(refreshToken) {
    try {
      // Verify refresh token
      const decoded = jwt.verify(refreshToken, process.env.JWT_SECRET);
   ...
   */
  async invalidateAllTokensService(userId) {
